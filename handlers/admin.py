...
def _parse_filename(filename):
    ...
    if se_match:
        ...
        title = re.sub(
            r"\b(720p|1080p|4K|WEBRip|BluRay|HDRip|x265|x264|HEVC|AAC|2CH)\b",
            "",
            name,
            flags=re.IGNORECASE
        ).strip()
...
async def add_file_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.args and len(context.args) >= 2:
        ...
        await update.effective_message.reply_text(
            "📤 Upload or forward any video/document to index it.\n"
            "Or: `/addfile <file_id> <title>`",
            parse_mode="Markdown"
        )
...
async def broadcast_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text(f"📡 Broadcasting to {len(user_ids)} users...")
